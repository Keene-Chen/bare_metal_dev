 ...
 * @return 		: 1 是闰年，0 不是闰年
 */
unsigned char rtc_is_leapyear(unsigned short year)
{
...
unsigned int rtc_coverdate_to_seconds(rtc_datetime_t* datetime)
{
    ...
    for (i = 1970; i < datetime->year; i++) {
        ...
        if (rtc_is_leapyear(i))
            days += 1; /* 闰年多加一天 		*/
    }

    days += monthdays[datetime->month];
    if (rtc_is_leapyear(i) && (datetime->month >= 3))
 ...
 * @return 			: 无
 */
void rtc_convertseconds_to_datetime(u64 seconds, rtc_datetime_t* datetime)
{
    u64 x;
    ...
    while (days > daysInYear) {
        ...
        /* 处理闰年 */
        if (!rtc_is_leapyear(datetime->year))
            daysInYear = DAYS_IN_A_YEAR;
        else /*闰年，天数加一 */
            daysInYear = DAYS_IN_A_YEAR + 1;
    }
    /*根据剩余的天数计算月份 */
    if (rtc_is_leapyear(datetime->year)) /* 如果是闰年的话2月加一天 */
        daysPerMonth[2] = 29;
