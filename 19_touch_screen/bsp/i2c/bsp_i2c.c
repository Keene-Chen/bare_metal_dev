...
unsigned char i2c_master_transfer(I2C_Type* base, i2c_transfer_t* xfer)
{
    ...
    if ((xfer->sub_addr_size > 0) && (xfer->direction == kI2C_Read)) {
        direction = kI2C_Write;
    }

    ret = i2c_master_start(base, xfer->slave_addr,
                           direction); // 发送开始信号
    if (ret) {
        return ret;
    }
    ...
    /* 发送寄存器地址 */
    if (xfer->sub_addr_size) {
        do {
            base->I2SR &= ~(1 << 1); // 清除标志位
            xfer->sub_addr_size--;   // 地址长度减一

            base->I2DR = ((xfer->sub_addr) >> (8 * xfer->sub_addr_size)); // 向I2DR寄存器写入子地址
        ...
        } while ((xfer->sub_addr_size > 0) && (ret == I2C_STATUS_OK));

        if (xfer->direction == kI2C_Read) // 读取数据
        {
            base->I2SR &= ~(1 << 1); // 清除中断挂起位
            i2c_master_repeated_start(base, xfer->slave_addr,
                                      kI2C_Read); // 发送重复开始信号和从机地址
            while (!(base->I2SR & (1 << 1))) {
            }; // 等待传输完成
    ...
    if ((xfer->direction == kI2C_Write) && (xfer->data_size > 0)) {
        i2c_master_write(base, xfer->data, xfer->data_size);
    }

    /* 读取数据 */
    if ((xfer->direction == kI2C_Read) && (xfer->data_size > 0)) {
        i2c_master_read(base, xfer->data, xfer->data_size);
    }
